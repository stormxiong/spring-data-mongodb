...
NOTE: By default, unless you specify a `MongoTransactionManager` within your application context, transaction support is **DISABLED**. You're required to set `setTransactionSychronizationEnabled` accordingly.
...
[source,java]
----
template.setTransactionSychronizationEnabled(true);                             <1>

// ...

TransactionTemplate txTemplate = new TransactionTemplate(txManager);            <2>
